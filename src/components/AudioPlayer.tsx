...
import {
  ...
  SkipForward, SkipBack, Loader2, Repeat, Gauge, Cloud, ExternalLink
} from 'lucide-react';
import { cn } from '@/utils/cn';
import { formatTime, getThumbnail, getCloudInfo, canDirectStream } from '@/types';
...
function ProviderBadge({ cloud }: { cloud: CloudInfo }) {
  return (
    <span className={cn(
      ...
      <Cloud className="h-2.5 w-2.5" />{cloud.label}
    </span>
  );
}
...
export function AudioPlayer({ item, onEnded, onNext, onPrev, accentColor = 'emerald', icon }: AudioPlayerProps) {
  ...
  // Check if we can stream directly (HTML5 audio) or need iframe
  const useDirectPlayer = useMemo(() => canDirectStream(cloud.provider), [cloud.provider]);

  useEffect(() => { setImgLoaded(false); setImgError(false); }, [item.id]);

  // Load new track (only for direct streams)
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio || !useDirectPlayer) return;
    const streamUrl = cloud.directUrl || cloud.embedUrl;
  ...
  }, [item.id, useDirectPlayer]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // CLOUD EMBED PLAYER — For Mega, GDrive, OneDrive, pCloud, etc.
  // Uses provider's iframe with their built-in controls
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  if (!useDirectPlayer) {
    const [embedLoading, setEmbedLoading] = useState(true);

    useEffect(() => { setEmbedLoading(true); }, [item.id]);

    const getHint = () => {
      switch (cloud.provider) {
        case 'mega': return '🔒 Mega.nz encrypted stream • Use Mega\'s controls';
        case 'gdrive': return '📁 Google Drive • Use Google\'s player controls';
        case 'onedrive': return '☁️ OneDrive • Use Microsoft\'s player controls';
        case 'pcloud': return '🌩 pCloud • Use pCloud\'s player controls';
        case 'sync': return '🔄 Sync.com • Use Sync\'s player controls';
        case 'internxt': return '🛡 Internxt • Use Internxt\'s controls';
        default: return 'Cloud stream • Use provider\'s controls';
      }
    };
        ...
        <div className="relative p-4 sm:p-5">
          ...
          <div className="relative rounded-xl overflow-hidden bg-black/40 border border-white/10 mb-4">
            {embedLoading && (
              <div className="absolute inset-0 flex items-center justify-center bg-black/40 z-10">
                <Loader2 className={cn("h-8 w-8 animate-spin", textAccent)} />
              </div>
            )}
            <iframe
              src={cloud.embedUrl}
          ...
          <div className="flex items-center justify-center gap-3">
            <button onClick={() => window.open(item.link, '_blank', 'noopener,noreferrer')}
              className={cn("flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-medium transition border", repeatBg, textAccent, "border-transparent hover:border-white/10")}>
          ...
          <div className="flex items-center justify-center gap-1.5 mt-2">
            <Cloud className={cn("h-3 w-3 opacity-25", cloud.textColor)} />
            <span className="text-[10px] text-white/30">{getHint()}</span>
          </div>
        </div>
      </div>
    );
  ...
  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  // DIRECT AUDIO PLAYER (MP3, Dropbox raw, direct links)
        ...
        <div className="relative flex items-center gap-3 sm:gap-4 mb-4 sm:mb-5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2 mt-1.5 flex-wrap">
              ...
              {cloud.provider !== 'direct' && <ProviderBadge cloud={cloud} />}
            </div>
          </div>
