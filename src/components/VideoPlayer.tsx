...
import {
  ...
  Monitor, AlertCircle, Youtube, Cloud, HardDrive, ExternalLink
} from 'lucide-react';
import { cn } from '@/utils/cn';
import {
  isYouTubeLink, getCloudInfo, canDirectStream,
  formatTime, getThumbnail
} from '@/types';
import type { MediaItem, CloudInfo } from '@/types';
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   PROVIDER BADGE — Shows cloud provider name + icon
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function ProviderBadge({ cloud }: { cloud: CloudInfo }) {
  const ProviderIcon = cloud.provider === 'youtube' ? Youtube
    : cloud.provider === 'direct' ? HardDrive
    : Cloud;

  return (
    <span className={cn(
      'shrink-0 rounded-md px-2 py-0.5 text-[10px] font-bold flex items-center gap-1 border',
      cloud.color, cloud.textColor, cloud.borderColor
    )}>
...
function YouTubePlayer({ item, onEnded, onNext, onPrev }: VideoPlayerProps) {
  ...
  const cloud = useMemo(() => getCloudInfo(item.link), [item.link]);

  useEffect(() => { expectedVideoId.current = videoId; }, [videoId]);
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      setLoading(true); setReady(false); setErrorState(false);
      setCurrentQuality(''); setPlayingSuggestion(false); setSuggestionTitle('');
      ...
      playerRef.current = new YT.Player(div, {
        ...
        height: '100%',
        playerVars: {
          controls: 1, modestbranding: 1, rel: 1, fs: 1,
        ...
        events: {
          onReady: () => {
            ...
            if (ytRef.current) ytRef.current.querySelectorAll('iframe').forEach(patchIframe);
          },
          onStateChange: (e: any) => {
            if (destroyed) return;
            ...
            switch (e.data) {
              case YT2.PlayerState.PLAYING: {
                setLoading(false);
                try { const q = playerRef.current?.getPlaybackQuality?.(); if (q) setCurrentQuality(q); } catch { /* */ }
                try {
                  ...
                  if (nowId && nowId !== expectedVideoId.current) {
                    ...
                    setPlayingSuggestion(false); setSuggestionTitle('');
                  }
                } catch { /* */ }
                break;
              }
              case YT2.PlayerState.BUFFERING: setLoading(true); break;
              case YT2.PlayerState.ENDED: { onEnded(); break; }
            }
          },
          onPlaybackQualityChange: (e: any) => { if (!destroyed) setCurrentQuality(e.data || ''); },
          onError: () => { if (!destroyed) { setLoading(false); setErrorState(true); } },
        },
      });
    };
  ...
  const toggleFullscreen = useCallback(async () => {
    ...
    if (!isCurrentlyFullscreen()) await enterFullscreen(c, null);
    else await doExitFullscreen();
  }, []);
  ...
  return (
    <div ref={containerRef}
      className={cn("relative overflow-hidden rounded-2xl bg-black shadow-2xl shadow-black/50 border border-white/[0.06]", isFullscreen && "rounded-none border-none")}
      onMouseMove={resetHideTimer} onMouseEnter={() => setShowOverlay(true)}>
      <div className={cn("relative w-full bg-black", isFullscreen ? "h-screen" : "aspect-video")}>
        <div ref={ytRef}
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between gap-3 px-3 sm:px-4 py-3 sm:py-4">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <ProviderBadge cloud={cloud} />
          </div>
          <div className="flex items-center gap-1 shrink-0 pointer-events-auto">
            <button onClick={onPrev} className="p-2 text-white/70 hover:text-white transition rounded-lg hover:bg-white/10 active:scale-95" title="Previous">
              <SkipBack className="h-4 w-4 sm:h-5 sm:w-5" fill="currentColor" />
            </button>
            <button onClick={onNext} className="p-2 text-white/70 hover:text-white transition rounded-lg hover:bg-white/10 active:scale-95" title="Next">
      ...
      {playingSuggestion && ready && (
        <div className="absolute bottom-14 inset-x-0 z-[6] flex justify-center px-3 pointer-events-none">
          <div className="flex items-center gap-3 rounded-xl bg-gradient-to-r from-violet-900/95 to-indigo-900/95 backdrop-blur-xl border border-violet-400/30 px-4 py-2.5 shadow-2xl shadow-violet-500/20 pointer-events-auto max-w-md w-full">
            ...
            <button onClick={() => {
              setPlayingSuggestion(false); setSuggestionTitle('');
              try { playerRef.current?.loadVideoById?.(expectedVideoId.current); } catch { /* */ }
      ...
      {ready && !errorState && !playingSuggestion && (
        <div className={cn('absolute bottom-2 left-3 z-[4] transition-all duration-500 pointer-events-none', showOverlay ? 'opacity-100' : 'opacity-0')}>
          <div className="flex items-center gap-1.5 rounded-lg bg-black/60 backdrop-blur-sm border border-white/10 px-2.5 py-1.5">
            ...
            <span className="text-[10px] text-white/50">⚙️ Quality • Click suggestions to play here</span>
          </div>
        </div>
      )}
    </div>
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   CLOUD EMBED PLAYER — For Mega.nz, Google Drive, OneDrive, pCloud,
   Sync.com, Internxt — Uses iframe embed with provider's own controls.
   Also used for Dropbox when direct streaming fails.
   ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */
function CloudEmbedPlayer({ item, onNext, onPrev }: VideoPlayerProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [loading, setLoading] = useState(true);
  const [showOverlay, setShowOverlay] = useState(true);
  ...
  const getHint = () => {
    switch (cloud.provider) {
      case 'mega': return '🔒 Mega.nz encrypted stream • Use Mega\'s controls for playback';
      case 'gdrive': return '📁 Google Drive stream • Use Google\'s player controls';
      case 'onedrive': return '☁️ OneDrive stream • Use Microsoft\'s player controls';
      case 'pcloud': return '🌩 pCloud stream • Use pCloud\'s player controls';
      case 'sync': return '🔄 Sync.com stream • Use Sync\'s player controls';
      case 'internxt': return '🛡 Internxt encrypted stream • Use Internxt\'s controls';
      default: return 'Cloud stream • Use provider\'s controls for playback';
    }
  };
    ...
    <div ref={containerRef}
      ...
      <div className={cn("relative w-full bg-black", isFullscreen ? "h-screen" : "aspect-video")}>
        ...
        {loading && (
          <div className="absolute inset-0 z-[3] flex items-center justify-center bg-black/60 pointer-events-none">
            <div className="flex flex-col items-center gap-3">
              <Loader2 className={cn("h-12 w-12 sm:h-14 sm:w-14 animate-spin", cloud.textColor)} />
              <span className="text-xs text-white/50">Loading from {cloud.label}...</span>
              {cloud.provider === 'mega' && <span className="text-[10px] text-white/30">Decrypting & streaming</span>}
            </div>
          </div>
        )}
      </div>
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between gap-3 px-3 sm:px-4 py-3 sm:py-4">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <Cloud className={cn("h-5 w-5 shrink-0", cloud.textColor)} />
            <p className="truncate text-sm sm:text-base font-semibold text-white">{item.title}</p>
            <ProviderBadge cloud={cloud} />
          </div>
          <div className="flex items-center gap-1 shrink-0 pointer-events-auto">
            <button onClick={openInNewTab}
              className="p-2 text-white/70 hover:text-white transition rounded-lg hover:bg-white/10 active:scale-95" title="Open in new tab">
      ...
      {!loading && (
        <div className={cn('absolute bottom-2 left-3 z-[4] transition-all duration-500 pointer-events-none', showOverlay ? 'opacity-100' : 'opacity-0')}>
          <div className="flex items-center gap-1.5 rounded-lg bg-black/60 backdrop-blur-sm border border-white/10 px-2.5 py-1.5">
            <Cloud className={cn("h-3 w-3", cloud.textColor, "opacity-50")} />
            <span className="text-[10px] text-white/50">{getHint()}</span>
          </div>
        </div>
      )}
    </div>
...
/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   DIRECT VIDEO PLAYER (MP4, Dropbox raw URLs)
...
function DirectVideoPlayer({ item, onEnded, onNext, onPrev }: VideoPlayerProps) {
  ...
  // Get the actual streamable URL
  const streamUrl = useMemo(() => {
    return cloud.directUrl || cloud.embedUrl;
  }, [cloud]);
  ...
  useEffect(() => {
    ...
    video.pause(); video.src = streamUrl; video.load();
    video.volume = volume; video.muted = false; video.playbackRate = playbackRate;
  }, [item.id, streamUrl]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Handle direct stream errors — fall back to CloudEmbed
  const [directFailed, setDirectFailed] = useState(false);
  useEffect(() => { setDirectFailed(false); }, [item.id]);

  if (directFailed && cloud.provider !== 'direct') {
    return <CloudEmbedPlayer item={item} onEnded={onEnded} onNext={onNext} onPrev={onPrev} />;
  }
  ...
  return (
    <div ref={containerRef}
      ...
      <div className={cn("relative w-full cursor-pointer", isFullscreen ? "h-full" : "aspect-video")}
        ...
        <video ref={videoRef}
          ...
          onLoadedMetadata={() => {
            const v = videoRef.current;
            if (v) { setDuration(v.duration); v.volume = muted ? 0 : volume; if (v.videoHeight > 0) setDetectedQuality(getResLabel(v.videoHeight)); }
            setLoading(false);
          }}
          onLoadedData={() => {
            const v = videoRef.current;
          ...
          onEnded={onEnded} onPlay={() => setPlaying(true)} onPause={() => setPlaying(false)}
          onProgress={updateBuffered}
          onError={() => {
            // If direct stream fails for cloud providers, fall back to embed
            if (cloud.provider !== 'direct') setDirectFailed(true);
          }}
        />
...
/* ━━━━━━━━━━━ MAIN EXPORT — Routes to correct player ━━━━━━━━━━━ */
export function VideoPlayer(props: VideoPlayerProps) {
  const cloud = getCloudInfo(props.item.link);

  // YouTube → dedicated YouTube IFrame API player
  if (isYouTubeLink(props.item.link)) return <YouTubePlayer {...props} />;

  // Providers that can give direct streamable URLs → HTML5 video player
  // (with automatic fallback to iframe if direct fails)
  if (canDirectStream(cloud.provider)) return <DirectVideoPlayer {...props} />;

  // All other cloud providers → iframe embed with provider's own controls
  // Mega, Google Drive, OneDrive, pCloud, Sync, Internxt
  return <CloudEmbedPlayer {...props} />;
}
